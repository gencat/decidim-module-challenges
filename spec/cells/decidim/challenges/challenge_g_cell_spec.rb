...
module Decidim::Challenges
  describe ChallengeCell, type: :cell do
    ...
    let(:model) { challenge }
    let(:cell_html) { cell("decidim/challenges/challenge_g", challenge, context: { show_space: show_space }).call }
    ...
    context "when rendering" do
      ...
      it "renders the card" do
        expect(cell_html).to have_css(".challenge-status")
      end
      ...
      it "renders the challenge image card" do
        expect(cell_html).to have_css(".card__grid-img")
      end
    end
  end
end
